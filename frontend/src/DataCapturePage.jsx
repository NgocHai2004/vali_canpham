...
export default function DataCapturePage({ go, initial, onDone, sessionId, sessionCode, sessionReadOnly = false, onSavedInSession, onEditProfile }) {
  ...
  useEffect(() => {
    ...
    return () => {
      stopped = true;
      // Roi trang: PHAI dung vong thu + dong session tren service. Truoc day chi
      // dat stopped=true (chan vong auto-start), nhung vong thu dang chay trong
      // startFpCollect van tiep tuc await capture() va session tren service van
      // mo => service giu thiet bi, vao lai trang thi startSession moi bi ket.
      fpAbortRef.current = true;
      fpRunningRef.current = false;
      const sid = fpSidRef.current;
      fpSidRef.current = null;
      // THU TU BAT BUOC: stopCapture() truoc, cancel() sau.
      // Lenh capture() dang cho nguoi dat tay moi la cai giu thiet bi; DELETE
      // session KHONG nha no (da test: sau DELETE, /api/live van active=true va
      // session moi bi 409 "Dang co lenh chup khac chay"). Chi capture/stop nha.
      fpApi.stopCapture()
        .catch(() => { /* noop */ })
