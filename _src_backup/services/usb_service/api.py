...
def _read_secret(name: str, dotenv_fallback) -> str:
    """Uu tien DPAPI store (.secrets.dat), fallback ve .env.

    usb_service nam sau 4 cap thu muc nen khong import truc tiep backend/
    secret_store.py duoc; nap bang importlib theo duong dan tuyet doi.
    Loi nap KHONG duoc lam sap service - con .env de fallback.
    """
    try:
        ...
        # Nap qua sys.path chu KHONG chi dinh "secret_store.py": sau khi dong bin
        # bang Nuitka --module thi tren dia chi con secret_store.pyd, tro dung
        # ten file .py se lam import that bai va service AM THAM tut ve .env.
        if backend not in sys.path:
            sys.path.insert(0, backend)
        mod = importlib.import_module("secret_store")
        return mod.get(name, dotenv_fallback)
    except Exception as e:  # noqa: BLE001
        print(f"[secret] fallback .env ({e})", flush=True)
        return dotenv_fallback(name)


_secret_str = _read_secret("DONGLE_SECRET", _load_dotenv_secret)
if not _secret_str:
    raise SystemExit(
